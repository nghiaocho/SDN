...
const createBook = async (req, res) => {
  try {
    ...
    if (typeof quantity !== 'number' || quantity < 0)
      return res.status(400).json({ message: "Quantity must be greater or equal 0" });
    ...
    if (existingBook) {
      ...
      return res.status(200).json({ message: "Quantity increased", book: existingBook });
    } else {
...
const deleteBook = async (req, res) => {
  try {
    ...
    if (activeRental) {
      return res.status(400).json({
        message: 'Cannot delete book: It is currently being borrowed or pending in an active rental.'
      });
    }
